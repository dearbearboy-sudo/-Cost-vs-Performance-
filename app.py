...
with st.expander("🔍 ตรวจสอบฐานข้อมูลฟิล์มและราคาอ้างอิงหลัก (Database Lookup)"):
...
# ส่วนที่ 1: การคำนวณและแสดงราคาตรวจสอบรายชั้นฟิล์ม
for i in range(int(num_layers)):
    ...
    with col3:
        manual_toggle = st.toggle("ปรับค่าวัสดุเอง", value=(selected_material == "กำหนดเอง (Custom Material)"), key=f"toggle_{i}")
    ...
    with col4:
        if manual_toggle:
            ...
            st.write(f"📊 ค่าคงที่: {price_per_kg} บาท/กก. | Density: {density} g/cm³")
    ...
    # 📝 สูตรหลักคำนวณราคาฟิล์มต่อตารางเมตร
    weight_per_m2 = actual_thickness * density  # หน่วย: g/m²
    film_cost_m2 = (weight_per_m2 / 1000) * price_per_kg  # หน่วย: บาท/m²
    total_film_cost += film_cost_m2

    # แสดงราคาสุทธิของชั้นนี้ทันทีเพื่อใช้เช็กความถูกต้องต่อหน้างาน
    st.markdown(f"**🔍 ผลลัพธ์คำนวณชั้นที่ {i+1}:** น้ำหนักฟิล์ม = `{weight_per_m2:.3f}` g/m² | **ราคาฟิล์มชั้นนี้ = `{film_cost_m2:.4f}` บาท/m²**")
...
if num_layers > 1:
    ...
    for j in range(num_adhesives):
        with col_adh[j]:
            ...
            st.caption(f"💰 ค่ากาวชั้นนี้: {adh_cost:.4f} บาท/m²")

st.write("---")

# ส่วนที่ 3: สรุปราคารวมทั้งหมดพ่วงด้วย Variable Cost 15%
total_material_cost = total_film_cost + total_adhesive_cost
variable_cost = 0.15 * total_material_cost
total_laminated_film_cost = total_material_cost + variable_cost
...
# ตารางแจกแจงโครงสร้างภายใน (ยังคงปิดการโชว์ราคาในตารางสรุปเพื่อความสะอาดตามความต้องการก่อนหน้า)
...
st.info("💡 หมายเหตุ: ตอนนี้ระบบได้กางสูตรแจกแจงน้ำหนักและราคารายชั้นฟิล์มออกมาให้เห็นชัดเจนอยู่ใต้แต่ละ Section แล้วเพื่อความแม่นยำในการตรวจสอบครับ")
